...
export default function TeamItem({ data, match, match_details, navigation, handleTeamShow }: { data: any, match: any, match_details: any, navigation: any, handleTeamShow: any }) {
    const { userToken, user } = useSelector((state: any) => state.user);
    return (
        <TouchableHighlight onPress={() => handleTeamShow(data)}>
            <View style={styles.wholeTeamContainer}>
                <View style={styles.teamName}>
                    <Text style={styles.bright}>{user?.username}({data?.teamId})</Text>
